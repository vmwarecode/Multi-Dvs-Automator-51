...
class NSXTAutomator:
    ...
    def option1_new_nsxt_instance(self, is_3x_4x_migration_env=False):
        ...
        self.utils.printCyan("Please Enter NSX VIP details")
        ...
        nsxt_1_fqdn = self.utils.valid_input("\033[1m Enter FQDN for 1st NSX Manager: \033[0m",
                                             None, self.__valid_fqdn)
        print(*three_line_separator, sep='\n')

        nsxt_2_fqdn = self.utils.valid_input("\033[1m Enter FQDN for 2nd NSX Manager: \033[0m",
                                             None, self.__valid_fqdn)
        print(*three_line_separator, sep='\n')

        nsxt_3_fqdn = self.utils.valid_input("\033[1m Enter FQDN for 3rd NSX Manager: \033[0m",
                                             None, self.__valid_fqdn)
        print(*three_line_separator, sep='\n')
        selected_option = "1"
        if not is_3x_4x_migration_env:
